...
export default function WinterVegetable() {
  ...
  useEffect(() => {
    const getVegetable = async () => {
      try {
        const winterVegi = await axios.get("/api/winterVegetable");
        setVegetable(winterVegi.data);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  .filter((item) => {
    ...
    return (
      <div>
        <Link to={`/winter/vegetable/${val._id}`}>
  ...
  return (
    <div className="winterVegeContainer w-full h-screen absolute top-0">
      <div
