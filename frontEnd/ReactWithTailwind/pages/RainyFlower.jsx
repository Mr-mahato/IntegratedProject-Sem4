...
export default function RainyFlower() {
  const [flower, setFlower] = useState([]);
  const [searchField , setSearchField] = useState('');
  useEffect(() => {
    const getFlower = async () => {
      try {
        const rainyFlower = await axios.get("/api/flower");
        console.log(rainyFlower.data)
        setFlower(rainyFlower.data);
      } catch (error) {
        console.log(error);
      }
    };
